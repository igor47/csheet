...
export const AbilityEditForm = ({ characterId, ability, currentScore, currentModifier, isProficient, proficiencyBonus, values, errors }: AbilityEditFormProps) => {
  ...
  const showPreview = (values?.score && parseInt(values.score) !== currentScore) || proficiencyChange !== 'none';
      ...
      >
        ...
        <div class="mb-3">
          ...
          <div class="border rounded p-2 text-center" style="max-width: 150px; margin: 0 auto;">
            <div class={clsx('fw-medium text-uppercase border', {
              'bg-primary-subtle': isProficient,
              'bg-dark-subtle': !isProficient,
            })} style="font-size: 0.7rem;">{ability}</div>
            <div class="fw-bold p-2">
              <span class="fs-4">{formatModifier(currentSavingThrow)}</span>
            </div>
            <div class="rounded-circle bg-secondary-subtle border d-flex align-items-center justify-content-center mx-auto fw-bold"
        ...
        {/* New Score Input */}
        <div class="mb-3">
          <label for="score" class="form-label">New Score (1-30)</label>
          <input
            type="number"
        ...
        <div class="mb-3">
          ...
          {errors?.proficiency_change && <div class="invalid-feedback d-block">{errors.proficiency_change}</div>}
        </div>
