...
export const CastSpellForm = ({
  ...
  if (!spell) {
    return (
      <ModalContent title="Spell Not Found"><div class="alert alert-danger">Spell not found</div></ModalContent>
    )
  }

  const isKnown = character.spells.some(s => s.knownSpells?.some(ks => ks === spell.id));
  const isPrepared = character.spells.some(s => s.preparedSpells.some(ps => ps.spell_id === spell.id) || s.cantripSlots.some(cs => cs.spell_id === spell.id));
  const hasSpellSlots = character.availableSpellSlots && character.availableSpellSlots.some(slot => slot >= spell.level);
  ...
  if (!isPrepared && !isKnown) {
    return (
      <ModalContent title="Cannot Cast"><div class="alert alert-danger">{character.name} cannot cast {spell.name}</div></ModalContent>
    );
  }
  ...
  const slotOptions = Object.values(slotLevelCounts).filter(s => s.level >= spell.level && s.available > 0).map(sl => ({
  ...
  <ModalContent title={title}>
    <div class="modal-body">
      ...
      <div class="alert alert-info mb-3">
        ...
        <strong>Note:</strong> This only tracks spell slot usage. Remember to manually apply spell effects (e.g., heal damage, apply conditions).
      </div>
