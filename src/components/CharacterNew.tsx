...
export const CharacterNew = ({ values, errors }: CharacterNewProps) => {
  const fields = [
    ...
    <div class="mb-3">
      ...
      <Select
        name="race"
        id="race"
        options={RaceNames.map((race) => (
          { value: race, label: toTitleCase(race) }
        ))}
  ...
  const subracePlh = subraces.length > 0 ? 'Select a subrace' : (selectedRace ? 'No subraces available' : 'Select a race first');
  fields.push(
    <div class="mb-3">
      ...
      <Select
        ...
        options={subraces.map((subrace) => (
          { value: subrace.name, label: toTitleCase(subrace.name) }
        ))}
  ...
  fields.push(
    <div class="mb-3">
      ...
      <Select
        name="background"
        id="background"
        options={BackgroundNames.map((bg) => (
          { value: bg, label: toTitleCase(bg) }
        ))}
          ...
          <div class="card-body">
            ...
            <form
              ...
              <div class="d-flex gap-2">
                <button type="submit" hx-post="/characters/new" class="btn btn-primary">Create Character</button>
