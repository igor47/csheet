...
function PrepareSpellFormBody({ character, values = {}, errors = {} }: PrepareSpellFormProps) {
  ...
  const hasOpenSlot = isCantrip
    ? selectedSI.cantripSlots.some((s) => s.spell_id === null)
    : selectedSI.preparedSpells.some((s) => s.spell_id === null)
  if (hasOpenSlot) {
    values.current_spell_id = ""
  }

  const currentlyPrepared = isCantrip
    ? selectedSI.cantripSlots.filter((s) => s.spell_id !== null)
    : selectedSI.preparedSpells.filter((s) => s.spell_id !== null)
  ...
  const replaceMessage =
    selectedSI.changePrepared === "levelup"
  ...
  return (
    <div class="modal-body">
      ...
      >
        ...
        <div class="mb-3">
          ...
          {onlyOneClass && (
            <>
              ...
              <small class="form-text text-muted">Only one spellcasting class available</small>
            </>
          )}
        ...
        <div class="mb-3">
          <fieldset class="btn-group" aria-label="Spell Type">
            <input
              type="radio"
              class="btn-check"
              name="spell_type"
  ...
  if (spellcastingSIs.length === 0) {
    return (
      <ModalContent title="Prepare Spell">
        <div class="alert alert-warning">{character.name} has no spellcasting classes.</div>
      </ModalContent>
    )
  }
