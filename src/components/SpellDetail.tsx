import type { Components, Spell } from "@src/lib/dnd/spells"
import {
  formatAreaOfEffect,
  formatCastingTime,
  formatDice,
    ...
    <div class={className}>
      <ul class="list-group list-group-flush mb-3">
        <li class="list-group-item">
          <strong>Spell Type:</strong> {levelText} {schoolText}
        </li>
        <li class="list-group-item">
        ...
        {spell.target && (
          <li class="list-group-item">
            <strong>Target:</strong> {spell.target.type === "self" && "Self"}
            {spell.target.type === "creature" && (
              <>
                {spell.target.count === "any"
                  ? "Any number of creatures"
                  : spell.target.count
                    ? `${spell.target.count} creature${spell.target.count > 1 ? "s" : ""}`
                    : "One creature"}
      ...
      <ul class="list-group list-group-flush">
        {spell.resolution.kind === "attack" && (
          <li class="list-group-item">
            ...
            {spell.resolution.attackType === "meleeSpell"
              ? "Melee Spell Attack"
              : spell.resolution.attackType === "rangedSpell"
                ? "Ranged Spell Attack"
                : spell.resolution.attackType === "meleeWeapon"
                  ? "Melee Weapon Attack"
        ...
        {spell.resolution.kind === "save" && (
          <li class="list-group-item">
            ...
            {spell.resolution.onSuccess && <span> (on success: {spell.resolution.onSuccess})</span>}
          </li>
        )}
        ...
        {spell.atHigherLevelsText && (
          <li class="list-group-item">
            <p>
              <strong>At Higher Levels:</strong> {spell.atHigherLevelsText}
            </p>
          </li>
        )}
