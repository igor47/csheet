...
import { SkillSchema, ProficiencyLevelSchema, type SkillType, type ProficiencyLevel } from "@src/lib/dnd";
...
export async function findByCharacterId(db: SQL, characterId: string): Promise<CharSkill[]> {
  ...
  return result.map((row: any) => CharSkillSchema.parse({
    ...row,
    created_at: new Date(row.created_at),
    updated_at: new Date(row.updated_at),
  }));
...
export async function currentByCharacterId(db: SQL, characterId: string): Promise<Record<SkillType, CurrentSkill>> {
  const result = await db`
    WITH ranked AS (
      SELECT
        skill,
  ...
  return result.reduce((acc: Record<SkillType, CurrentSkill>, row: any) => {
