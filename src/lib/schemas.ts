...
export const NumericSelectFormFieldSchema = z.literal("").transform(() => undefined).or(NumberFormFieldSchema)
...
export function StringNumberEnum<const T extends readonly number[]>(values: T) {
  return z.preprocess(
    ...
    (v) => values.includes(v as (typeof values)[number]),
